...
static LCSocketManager *_instance = nil;
...
+ (instancetype)share {
    ...
    dispatch_once(&onceToken, ^{
        _instance = [[LCSocketManager alloc] init];
        [_instance initScoket];
        [_instance pullMsg];
    });
...
- (void)pullMsg {
    //新开辟了一个常驻线程，在这个线程中一个死循环里去不停的调用recv函数，这样服务端有消息发送过来，第一时间便能被接收到。

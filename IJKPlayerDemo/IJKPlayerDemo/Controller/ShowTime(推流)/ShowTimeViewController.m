...
@interface ShowTimeViewController ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate>
...
@property (nonatomic, weak) UIImageView *focusCurorImageView;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"视频采集";
    if (![LiveCommon getCamareLimits] || ![LiveCommon checkAudioStatus])  return;
...
{
    if (_focusCurorImageView == nil) {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"focus"]];
        _focusCurorImageView = imageView;
        [self.view addSubview:_focusCurorImageView];
    }
    return _focusCurorImageView;
}

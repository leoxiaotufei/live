...
{
  @public
	NSMutableData *buffer; //当前包的数据 ，（容器，有可能为空）
	NSUInteger startOffset; //开始偏移 （数据在容器中开始写的偏移）
	NSUInteger bytesDone; //已读字节数 （已经写了个字节数）
	NSUInteger maxLength; //想要读取数据的最大长度 （有可能没有）
	NSTimeInterval timeout; //超时时长
	NSUInteger readLength; //当前需要读取总长度  （这一次read读取的长度，不一定有，如果没有则可用maxLength）
	NSData *term; //包的边界标识数据 （可能没有）
    //判断buffer的拥有者是不是这个类，还是用户。
    //跟初始化传不传一个buffer进来有关，如果传了，则拥有者为用户 NO， 否则为YES
	BOOL bufferOwner;
	NSUInteger originalBufferLength; //原始传过来的data长度
	long tag;
}
...
@implementation GCDAsyncSocket
{
	...
	uint32_t flags; //flags，当前正在做操作的标识符
	uint16_t config;
	...
	int socketUN; //unix域的套接字
	NSURL *socketUrl; //unix域 服务端 url
	...
	dispatch_queue_t socketQueue; //这个类的对Socket的操作都在这个queue中，串行队列
	
	dispatch_source_t accept4Source;
	dispatch_source_t accept6Source;
	dispatch_source_t acceptUNSource;
    
	dispatch_source_t connectTimer;  //连接timer,GCD定时器
	...
	GCDAsyncReadPacket *currentRead;  //当前正在读写数据包
	GCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable; //当前socket未获取完的数据大小
	
	GCDAsyncSocketPreBuffer *preBuffer; //全局公用的提前缓冲区
	...
	void *IsOnSocketQueueOrTargetQueueKey; //socket队列的标识key
	
	id userData;
    NSTimeInterval alternateAddressDelay; //连接备选服务端地址的延时 （另一个IPV4或IPV6）
}
...
- (id)initWithDelegate:(id<GCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		socket6FD = SOCKET_NULL; //ipv6
		socketUN = SOCKET_NULL; //应该是UnixSocket
		socketUrl = nil;
		stateIndex = 0; //状态
                ...
                error:(NSError **)errPtr
{
	...
	__block BOOL result = NO;
    __block NSError *preConnectErr = nil; //error信息
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if ([host length] == 0)
		{
			...
			return_from_block; //其实就是return,大牛的代码真是充满逼格
		}
		...
		NSString *hostCpy = [host copy];    //copy，防止改变
		
        int aStateIndex = self->stateIndex;//拿到状态
		__weak GCDAsyncSocket *weakSelf = self;
		...
		#pragma clang diagnostic warning "-Wimplicit-retain-self"
			...
			__strong GCDAsyncSocket *strongSelf = weakSelf;
            if (strongSelf == nil) return_from_block; //完整Block安全形态，在加个if
	...
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
		block();
    else    //否则同步的调起这个queue去执行

		dispatch_sync(socketQueue, block);
	
	//如果有错误，赋值错误
	if (errPtr) *errPtr = preConnectErr;
	return result; //把连接是否成功的result返回
}
...
- (int)createSocket:(int)family connectInterface:(NSData *)connectInterface errPtr:(NSError **)errPtr
{
    ...
    if (socketFD == SOCKET_NULL) //如果创建失败
    {
        if (errPtr)
     ...
     //和connectInterface绑定.Socket绑定本机地址
    if (![self bindSocket:socketFD toInterface:connectInterface error:errPtr])
    {
        //绑定失败，直接关闭返回
        [self closeSocket:socketFD]; //绑定失败，直接关闭返回
...
- (void)connectSocket:(int)socketFD address:(NSData *)address stateIndex:(int)aStateIndex
{
    ...
    dispatch_queue_t globalConcurrentQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
    dispatch_async(globalConcurrentQueue, ^{//获取到全局Queue
#pragma clang diagnostic push
#pragma clang diagnostic warning "-Wimplicit-retain-self"
                        ...
                        port:(uint16_t)port
{
	...
	if (interface == nil)
	{
        ...
        sockaddr4.sin_family      = AF_INET;  //addressFamily IPv4(AF_INET) 或 IPv6(AF_INET6)。

        sockaddr4.sin_port        = htons(port);   //端口号 htons将主机字节顺序转换成网络字节顺序  16位
	...
	{ //如果当前读的包为空，而且flag为已连接
		...
		{//如果有安全socket
			...
			{ //如果可读字节数为0
				...
				else {  //重新恢复读的source。因为每次开始读数据的时候，都会挂起读的source
					[self resumeReadSource];
				}
			}
		}
